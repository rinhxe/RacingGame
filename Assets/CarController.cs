...
public class CarController : MonoBehaviour
{
    ...
    // Thêm biến điều chỉnh lực hãm tự nhiên (ma sát lăn)
    ...
    void CheckInput()
    {
        // Sử dụng input map mới với 4 action: Up, Down, Left, Right (qwes)
        gasInput = 0f;
        steeringInput = 0f;
        if (inputActions.Move.Up.ReadValue<float>() > 0.5f) // Q
        {
            gasInput = 1f;
        }
        else if (inputActions.Move.Down.ReadValue<float>() > 0.5f) // S
        {
            gasInput = -1f;
        }
        ...
        if (inputActions.Move.Right.ReadValue<float>() > 0.5f) // E
        {
            steeringInput = 1f;
        }
        else if (inputActions.Move.Left.ReadValue<float>() > 0.5f) // W
        {
            steeringInput = -1f;
        }
        ...
        if (gearState != GearState.Changing)
        {
            ...
            {
                clutch = Keyboard.current.leftShiftKey.isPressed ? 0 : Mathf.Lerp(clutch, 1, Time.deltaTime);
            }
        }
        else
        {
    ...
    void ApplyBrake()
    {
        ...
        // Nếu không có input ga và phanh, áp dụng lực hãm tự nhiên lên bánh sau
        if (Mathf.Abs(gasInput) < 0.01f && brakeInput == 0 && speed > 0.5f)
        {
            float resistance = rollingResistance * brakePower;
            colliders.RRWheel.brakeTorque += resistance;
            colliders.RLWheel.brakeTorque += resistance;
        }
